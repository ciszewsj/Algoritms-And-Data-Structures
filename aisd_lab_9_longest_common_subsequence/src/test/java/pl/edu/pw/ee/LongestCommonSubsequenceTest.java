...
public class LongestCommonSubsequenceTest {
	...
	private void test_method_compare(String[] strings, String template1, String template2) {
		...
		assertEquals(longestCommonSubsequence.findLCS(), template1);

		LongestCommonSubsequence longestCommonSubsequenceReverse = new LongestCommonSubsequence(strings[1], strings[0]);
		assertEquals(longestCommonSubsequenceReverse.findLCS(), template2);
	}

	private void display_method_compare(String[] strings, String templateOfDisplay) {
		LongestCommonSubsequence longestCommonSubsequence = new LongestCommonSubsequence(strings[0], strings[1]);
		assertEquals(longestCommonSubsequence.toString(), templateOfDisplay);

	}

	@Test
	public void ababab_test() {
		test_method_compare(ababab_test, ababab_result, ababab_reverse_result);
	}

	@Test
	public void politechnika_toaleta_test() {
		test_method_compare(politechnika_test, politechnika_result, politechnika_reverse_result);
	}

	@Test
	public void polish_chars_test() {
		test_method_compare(polish_chars_test, polish_chars_result, polish_chars_reverse_result);
	}

	@Test
	public void strange_chars_test() {
		test_method_compare(strange_chars_test, strange_chars_result, strange_chars_reverse_result);
	}

	@Test
	public void one_string_free_test() {
		test_method_compare(one_string_free_test, one_string_free_result, one_string_free_reverse_result);
	}
